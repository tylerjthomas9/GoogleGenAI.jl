...
if haskey(ENV, "GOOGLE_API_KEY")
    ...
    @testset "Content Caching" begin
        ...
        response_cached = generate_content_with_cache(secret_key, model, cache_name, single_prompt)
        @test response_cached.response_status == 200
    
        # 3) Generate content with the cache (conversation)
        conversation = [
            Dict(:role => "user", :parts => [Dict(:text => "We previously discussed some text. Summarize it again.")])
        ]
        response_convo = generate_content_with_cache(secret_key, model, cache_name, conversation)
        ...
        update_result = update_cached_content(secret_key, cache_name; ttl="90s")
